...
void fifo(int processes[], int n, int burst_time[n]) {
    int turnaround_time[n], response_time[n]; // Arrays for turnaround and response times
    ...
    for (int i = 1; i < n; i++) {
        response_time[i] = response_time[i - 1] + burst_time[i - 1]; // Response time is the sum of previous burst times
        turnaround_time[i] = response_time[i] + burst_time[i]; // Turnaround time = response time + burst time
    ...
    printf("Process\tBurst Time\tTurnaround Time\tResponse Time\n");
    for (int i = 0; i < n; i++)
        printf("%d\t%d\t\t%d\t\t%d\n", processes[i], burst_time[i], turnaround_time[i], response_time[i]);
...
int main() {
    ...
    fifo(processes, n, burst_time); // Call the FIFO function
